...
for file in files:
    ...
    if deps.get("lines", 0) > 500:
        issues["large_files"].append(
            {"file": file_str, "lines": deps["lines"], "functions": deps["functions"]}
        )
    ...
    if unused:
        issues["unused_imports"].append(
            {"file": file_str, "count": len(unused), "imports": unused}
        )
...
result = {
    ...
    "total_lines": sum(
        d.get("lines", 0) for d in [analyze_dependencies(str(f)) for f in files]
    ),
    "issues": {
