...
1. Count: "Found 15 occurrences in 5 files"
1. Replace in each file with `replace_all=true`
1. Verify: Re-run Grep
1. Suggest: Run tests

### Replace Deprecated Pattern

1. Find: `Grep(pattern="\\bvar\\s+\\w+", output_mode="content", -n=true)`
1. Analyze: Check if reassigned (let) or constant (const)
1. Replace: `Edit(old_string="var count = 0", new_string="let count = 0")`
1. Verify: `npm run lint`

### Update API Calls

1. Find: `Grep(pattern="/api/auth/login", output_mode="content", -n=true)`
1. Replace: `Edit(old_string="'/api/auth/login'", new_string="'/api/v2/authentication/login'", replace_all=true)`
1. Test: Recommend integration tests
...
1. Search → Find all
1. Analyze → Verify appropriate
1. Inform → Tell user scope
1. Execute → Make changes
1. Verify → Confirm applied
1. Test → Suggest running tests
