...
```bash
# --- Output format ---
docling report.pdf --output /tmp/                          # Markdown (default)
docling report.pdf --to json --output /tmp/               # JSON (structured)

# --- Pipeline selection ---
docling report.pdf --pipeline vlm --output /tmp/           # VLM for complex layouts
docling report.pdf --pipeline vlm --vlm-model granite_docling --output /tmp/  # VLM with specific model

# --- OCR options ---
docling report.pdf --ocr-engine tesserocr --output /tmp/  # Custom OCR engine
docling report.pdf --no-ocr --output /tmp/                # Disable OCR (speed)

# --- Speed / feature toggles ---
docling report.pdf --no-tables --output /tmp/             # Skip table extraction
...
# Requires: uv add 'docling-core[chunking-openai]'
```
...
2. Re-convert with `docling`, re-run `uv run scripts/docling-evaluate.py`.
...
```bash
# Install the docling CLI as a uv tool
uv tool install docling
# For OpenAI tokenizer support, add to your project:
...
`scripts/docling-evaluate.py` declares its own dependencies as inline script
metadata (PEP 723) — run it directly with `uv run`, no install step needed:

```bash
uv run scripts/docling-evaluate.py /tmp/report.json --markdown /tmp/report.md
```

## Example

For a worked end-to-end walkthrough see [EXAMPLE.md](EXAMPLE.md).
